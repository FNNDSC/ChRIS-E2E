...
if CAPTURE:
    ...
    PID = int(stdout)
    subprocess.call(["top -p " + str(PID) + " -d 0.2 -b | awk '{print $9, $10}' >> /tmp/top.log &"], shell=True)
    # time.sleep(2)
    start = time.time()
...
for x in range(RANGE):
    ...
    try:
        response = json.loads(json.loads(stdout))
...
for z in dirs:
    cmd = 'bash %s/ChRIS-E2E/scripts/run_pman %s %s %s %s' % (PATH, z, PMAN_IP, f'TestService{throwaway_Var}', IMAGE)
    process = Popen(shlex.split(cmd), stdout=PIPE, stderr=PIPE, shell=False, close_fds=True)
    stdout, stderr = process.communicate()
...
while (incomplete != [] and wait_time <= MAX_DELAY):
    ...
    for job in incomplete:
        ...
        try:
            response = json.loads(json.loads(stdout))
            if len(response['d_ret']['l_status']) and ("finished" in ((response['d_ret']['l_status'])[0])):
                incomplete.remove(job)
                pman_success += 1
                print("Job " + str(job) + " succeeded!")
            elif len(response['d_ret']['l_status']) and (((response['d_ret']['l_status'])[0]) == "started"):
